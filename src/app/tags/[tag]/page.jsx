...
export default function Page({ params }) {
  ...
  const {
    ...
    queryFn: () =>
      fetch(`
       ${config.url}/api/tag?tag=${tagLabel?.value}&limit=50&page=${page ? page : 1
            ...
            <Flex vertical className="w-full max-w-[100%]">
              ...
              <Flex justify="center" gap={50} className="w-full mt-[20px]">
                <button
                  ...
                  className={`p-2 md:p-3 px-4 md:px-7 rounded-md text-sm md:text-base bg-[#00803e]  text-white ${(!page || Number(page) < 2) &&
                ...
                <button
                  ...
                  onClick={handleNext}
                  className={`p-2 md:p-3 px-4 md:px-7 rounded-md text-sm md:text-base bg-[#0047A0] text-white ${tagData?.length < 50 && "bg-[#0047A0] cursor-not-allowed"
                    }`}
                >
                  Next
                </button>
