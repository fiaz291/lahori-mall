...
const handlePost = async (req, res) => {
  const { userId, orderItems, orderAddress, billingAddress, vouchers } = req.body;
...
const handleGet = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json(createResponse({data:{orders, totalPages} }));
  } catch (error) {
    console.error(error);
    return res.status(500).json(createResponse({ message: "Internal Server Error" }));
  }
};
